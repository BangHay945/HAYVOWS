...
export function UpgradeModal({
  ...
  const [selectedPlan, setSelectedPlan] = useState<"premium" | "luxury">("premium");
    ...
    try {
      ...
      if (typeof window !== "undefined" && (window as any).snap && !isSimulated) {
        ...
        if (confirmSimulate) {
          ...
          if (simRes.ok) {
            alert(`Selamat! Akun Anda berhasil di-upgrade ke Paket ${selectedPlan.toUpperCase()}!`);
            onUpgradeSuccess?.();
            onClose();
            window.location.reload();
          }
  ...
  const whatsappMessage = encodeURIComponent(
    `Halo Admin Hayvows, saya ingin konfirmasi upgrade ke Paket ${selectedPlan.toUpperCase()} (Rp ${
      selectedPlan === "premium" ? "199.000" : "299.000"
    }). Mohon bantuannya.`
  );

  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 max-w-2xl w-full overflow-hidden flex flex-col my-8 animate-in fade-in zoom-in-95 duration-200 text-slate-800">
        {/* Header */}
        <div className="p-6 pb-4 bg-gradient-to-b from-[#faf8f5] to-white border-b border-slate-100 flex items-start justify-between">
          <div>
            ...
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 mt-2">
              Upgrade Paket Undangan Anda
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              Hapus label mode uji coba, buka tema eksklusif, dan kirim ke ribuan tamu tanpa batas.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div
              ...
              className={`p-5 rounded-2xl border-2 transition-all cursor-pointer relative ${
                selectedPlan === "premium"
                  ? "border-[#2d4a3e] bg-emerald-50/30 shadow-md ring-2 ring-[#2d4a3e]/15"
            ...
            >
              ...
              <h3 className="font-bold text-slate-900 text-lg">Paket Populer</h3>
              <p className="text-2xl font-extrabold text-[#2d4a3e] mt-1">
                Rp 199.000
                <span className="text-xs font-normal text-slate-400"> / sekali bayar</span>
              </p>

              <div className="mt-4 pt-3 border-t border-slate-100 space-y-2 text-xs text-slate-600">
                <div className="flex items-center gap-2">
                  <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                  <span className="font-semibold text-slate-800">Tema Nature Floral &amp; Batik Jawa</span>
                </div>
                <div className="flex items-center gap-2">
                  <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                ...
                <div className="flex items-center gap-2">
                  <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                  <span>Amplop Digital &amp; Salin Cepat</span>
                </div>
                <div className="flex items-center gap-2">
                  <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                  <span>Bebas Pasang Musik Sendiri (MP3/YouTube)</span>
                </div>
                <div className="flex items-center gap-2">
                  <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                  <span>Kategori Tamu VIP &amp; Kuota Pax Tamu</span>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              className={`p-5 rounded-2xl border-2 transition-all cursor-pointer relative ${
                selectedPlan === "luxury"
                  ? "border-[#c9a84c] bg-amber-50/20 shadow-md ring-2 ring-[#c9a84c]/20"
                  : "border-slate-200 hover:border-slate-300 bg-white"
              }`}
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-[#0a0a0a] text-[#c9a84c] uppercase tracking-wider border border-[#c9a84c]/40 flex items-center gap-1">
              ...
              <p className="text-2xl font-extrabold text-[#c9a84c] mt-1">
                Rp 299.000
                <span className="text-xs font-normal text-slate-400"> / sekali bayar</span>
              </p>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 space-y-2 text-xs text-slate-600">
                ...
                <div className="flex items-center gap-2">
                  <Check className="w-3.5 h-3.5 text-[#c9a84c] shrink-0" />
                  <span className="font-semibold text-slate-800">Tema 2D RPG &amp; Cyberpunk 2077</span>
                </div>
                <div className="flex items-center gap-2">
                  <Check className="w-3.5 h-3.5 text-[#c9a84c] shrink-0" />
                  <span>Tiket E-Pass QR &amp; Layar Sapa Digital</span>
                </div>
                <div className="flex items-center gap-2">
                  <Check className="w-3.5 h-3.5 text-[#c9a84c] shrink-0" />
                  <span>Mode Split-Screen Interaktif &amp; NPC</span>
                </div>
              </div>
            </div>
          </div>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <span>
                    Bayar Instan dengan Midtrans (QRIS / Virtual Account)
                  </span>
