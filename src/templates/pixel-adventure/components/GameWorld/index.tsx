...
export function PixelGameWorld({
  ...
  // Viewport tracking for RPG follow camera
  ...
  // Player position in percentage coordinates
  const [playerPos, setPlayerPos] = useState({ x: 50.15, y: 96.0 });
  const posRef = useRef({ x: 50.15, y: 96.0 });
  ...
  // Wooden RPG Dialogue Box State & Modals
  ...
  // Search params resolution for guest name
  ...
  // World Sizing for Full-Page RPG Map (Native 1376 x 768)
  const isMobile = viewportSize.width < 768;
  const MAP_ASPECT = 1376 / 768;
  const zoomFactor = isMobile ? 1.5 : 1.2;

  let baseHeight = Math.max(viewportSize.height * zoomFactor, isMobile ? 800 : 860);
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (isInputActive) {
        ...
        if (isWalkable(curX + stepX, curY + stepY)) {
          candidateX = curX + stepX;
          candidateY = curY + stepY;
        } else if (isWalkable(curX + stepX, curY)) {
          candidateX = curX + stepX;
          candidateY = curY;
        } else if (isWalkable(curX, curY + stepY)) {
          candidateX = curX;
          candidateY = curY + stepY;
        }
  ...
  return (
    <div className="relative w-screen h-[100dvh] overflow-hidden select-none font-pixel bg-[#160309]">
      <MapRenderer
